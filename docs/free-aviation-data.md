...
| **OurAirports** | Fed the whole map layer at first, and now supplies **communication frequencies only**. Its `gps_code` turned out to be the local code repeated for most airports — `ID15`, `WN43` — strings that can never match a METAR, so the airport facts moved to the FAA. Its frequency table is still the better one: 3,752 airports against the FAA's 2,493 services. |
| **FAA AIS ArcGIS** | Now the source for airports, runways *and* airspace, all on the same 28-day AIRAC cycle. `US_Airport`, `Runways`, `Class_Airspace`. See [airspace-and-airports.md](airspace-and-airports.md). |
| **BTS `data.bts.gov`** | Authoritative NASR-derived facility data, and **frozen at 2020-07-16**. There is no current facility dataset on that portal — the rest are traffic and on-time statistics. Six-year-old airport data is worse than a community source that is edited continuously. |
